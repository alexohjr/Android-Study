package com.and.andelectronics.common.ez;
...
public class EZAlertDialog extends DialogFragment implements View.OnClickListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        if (_buttons != null) {
            for (int i=0; i<_buttons.length; i++) {
                ...
                // 버튼 추가
                LinearLayout ll = (LinearLayout)inflater.inflate(R.layout.view_alert_btn, null);
                Button btn = (Button)ll.findViewById(R.id.v_alert_btn);
                btn.setText(_buttons[i]);
                btn.setId(i);
                btn.setOnClickListener(this);
                _containerLayout.addView(ll);
            }
            rootView.findViewById(R.id.d_alert_confirmBtn).setVisibility(View.GONE);
        }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.d_alert_confirmBtn:
                if (_listener != null) {
                    if (_buttons != null) {
                        LogUtil.d(TAG, "clicked confirm with index");
                        _listener.onClickConfirm(_selectedButtons);
                    } else {
                        LogUtil.d(TAG, "clicked confirm");
    ...
    public static void showSimpleDialog(final Context ctx, final String title, final String message, final EZAlertDialogListener listener) {
        Handler handler = new Handler(Looper.getMainLooper());
        handler.post(new Runnable() {
            @Override
            public void run() {
                ...
                builder.setPositiveButton(ctx.getString(R.string.comm_ok), new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int id) {
                        ...
                        .setNegativeButton(ctx.getString(R.string.comm_cancel), new DialogInterface.OnClickListener() {
                            @Override
