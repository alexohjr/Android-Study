...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 리스트 뷰의 오버스크롤 효과 (다 올리면 파란색 그림자)를 제거
        ...
        searchEt.setOnEditorActionListener(new TextView.OnEditorActionListener() {
            ...
            public boolean onEditorAction(TextView v, int actionId, KeyEvent event) {
                if(actionId == EditorInfo.IME_ACTION_SEARCH) {
                    ...
                    // 참고) android:imeOptions="actionSearch"로 설정되었다면 엔터키를 눌렀을 때
                    // 돋보기 버튼 눌림

                    // 검색버튼(searchBtn)의 클릭 이벤트 실행
                    searchBtn.performClick();
                }

                // if()를 없애면 (에뮬레이터 상에서)엔터 쳐도 검색 실행됨
                return true;
            }
        });

        // footer_view를 클릭하면 추가 로드 되도록
    ...
    class NaverAsync extends AsyncTask<String, String, ArrayList<BookModel>> {
        ...
        protected void onPostExecute(ArrayList<BookModel> bookModels) {
            ...
            Log.d("MY", "onPostExecute() 실행");
            if(adapter == null) {
                adapter = new MyViewModelAdapter(MainActivity.this, R.layout.book_item, bookModels, listView);
    ...
    AbsListView.OnScrollListener scrollListener = new AbsListView.OnScrollListener() {
        ...
        public void onScroll(AbsListView view, int firstVisibleItem, int visibleItemCount, int totalItemCount) {
            ...
            //               : true(추가 로드 안함 = > 리스트뷰 구현 끝냄)
            if(firstVisibleItem >= count && totalItemCount != 0 && !mLockListView) {
                // firstVisibleItem >= count : 스크롤 뷰의 맨 밑부분인지 여부
                ...
                if(start < 1000 - 10 && list.size() >= 10) {
                    ...
                    // start가 책 아이템 개수를 벗어나면
                    if(start >= list.size()) {
                        // start 를 강제로 990으로 만들기 (추가 로드 못하게)
                        start = 1000 - 10;
                    ...
                    Toast.makeText(MainActivity.this, "더 이상 불러올 내용이 없습니다.", Toast.LENGTH_SHORT).show();
